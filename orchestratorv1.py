# orchestrator.py (Claude Bedrock Integration + Confluence Publishing)
...
from src.diagram_generator import generate_mermaid_from_requirements
from src.confluence_uploader import publish_to_confluence
...
async def run_workflow_async(file_path: str, keyword: str):
    ...
    # Save JIRA story content for diagram generation
    with open("generated/jira_output.txt", "r") as f:
        jira_story_text = f.read()

    # Generate Mermaid diagram and push to Confluence
    mermaid_code = generate_mermaid_from_requirements(jira_story_text)
    with open("generated/requirement_diagram.mmd", "w") as f:
        f.write(mermaid_code)
    publish_to_confluence("SDLC Requirement Diagram", mermaid_code)
...
async def run_supervised_step(step: str, target_agent, user_input: str = None):
    ...
    groupchat = GroupChat(
        ...
        llm_config={"model": "bedrock.claude-v2"}
    )
